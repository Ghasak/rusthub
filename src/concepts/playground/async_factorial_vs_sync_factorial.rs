...
fn factorial_async_multithreaded(n: u128) -> u128 {
    if n == 0 {
        ...
        for i in 0..num_threads {
            ...
            let end = if i == num_threads - 1 { n } else { (i as u128 + 1) * chunk_size };
...
fn compare_factorial_results(n: u128) {
    ...
    println!("Factorial of {} using sync: {} (time: {}us)", n, result_sync, elapsed_sync);

    let now = Instant::now();
    let result_async_mt = factorial_async_multithreaded(n);
    let elapsed_mt = now.elapsed().as_micros();
    println!("Factorial of {} using async and multi-threading: {} (time: {}us)", n, result_async_mt, elapsed_mt);
}
